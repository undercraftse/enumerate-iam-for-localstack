## Note
I've taken a good project by Andres Riancho and ruined it, in order to get it to sort of barely work for CTFs/localstack/Emulation environments.
Things I've done
- Modified to allow endpoint urls in the commandline and added them to the relevant brute force functions.
- Removed the enumerate_using_iam function, in favour of just having brute force. Due to not wanting to deal with the errors.
- Put in terrible try/except statements to try to minimise the errors seen due to parsing issues between the emulation environment and what boto expects.

I'd advise against using this as a sole source of AWS permission enumeration but it may provide some additional initial information.
